...
var sc = $('#seat-map').seatCharts({
  ...
  click: function click() {
    if (this.status() == 'available') {
      $('<li>Silla # ' + this.settings.id + ': <a href="#" class="cancel-cart-item">[Borrar]</a></li>').attr('id', 'cart-item-' + this.settings.id).data('seatId', this.settings.id).appendTo($cart);
      $counter.text(sc.find('selected').length + 1);
      return 'selected';
    } else if (this.status() == 'selected') {
      $counter.text(sc.find('selected').length - 1); //remove the item from our cart
    ...
    success: function success(result) {
      if (result.length > 0) {
        for (var i = 0; i < result.length; i++) {
          var element = result[i]; //format of seats for jquery plugin
    ...
    $.ajax({
      ...
      error: function error(_error) {
        ...
        if (allErrorMessages.length > 0) {
          ...
          alert(_error.responseJSON.message + ": " + errorString);
        } else {
...
function setDatesDropDown() {
  ...
  reservationDateDropDown.append($('<option></option>').val('none').html('Seleccione una fecha'));
  /**
   * adding options
   */
  ...
  for (var i = 0; i < dates.length; i++) {
    var element = dates[i];
    reservationDateDropDown.append($('<option></option>').val(element).html(element));
  }
  /**
...
/***/ 1:
/*!*************************************!*\
  !*** multi ./resources/js/seats.js ***!
  \*************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! D:\installations\xampp\htdocs\lara\resources\js\seats.js */"./resources/js/seats.js");
